    ...
    private void redrawOrderItems(Order thisOrder, Scanner scan) {
        ...
        while (true) {
            ...
            System.out.printf("%-35s %15s %9s %8s %16s %5s\n", "Product Name", "Category", "Unit Price", 
                "Quantity", "Total Price", "ID");
            int countItem = 1;
            for (OrderItem item : orderItems) {
                int price = item.getUnitPrice();
                int quantity = item.getQuantity();
                long linePrice = price*(long)quantity;
                System.out.printf("%-35s %15s %9s %8s %16s %5d\n", item.getProductName(),
                    item.getProductCategory(), PriceUtil.centsToString(price), quantity,
                    PriceUtil.centsToString(linePrice), countItem);
                ++countItem;
            }
            System.out.println("==============================================================");
